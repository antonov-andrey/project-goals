...
В рамках явно запрошенного пользователем real testing эта task вправе в exact bound destination создать или принять только утверждённые missing statuses `Human Review`, `Rework`, `Merging`, role labels, label `agent:codex`, требуемые Project statuses и bounded isolated acceptance Project family с его issues, relations, documents и comments. Family содержит один complete-flow Project и отдельный cancellation-path Project только когда один Project не может доказать обе terminal Project semantics без запрещённого reopen. Existing standard statuses принимаются по semantic category и не переименовываются, не удаляются и не создаются повторно. Foreign labels, statuses, Projects и другие workspace settings не изменяются. Planned global delta всё равно показывается в execution output, но отдельное повторное разрешение на тот же exact closed mutation set не требуется.

Каждый acceptance Project получает user-visible name с префиксом `Acceptance` и provider-owned identity из exact team ID, source fingerprint, exact candidate fingerprint и path role `complete` либо `cancellation`. Retry одного non-terminal acceptance run reconciles тот же Project вместо создания duplicates. `Completed` и `Canceled` Projects никогда не reopen-ятся; новый terminal re-execution для другого candidate или нового run получает новую exact identity. Complete-flow Project и его issues сохраняются как `Completed` history, cancellation-path Project сохраняется как `Canceled` history. Cleanup удаляет только declared Git, local и external runtime resources и никогда не удаляет Linear history.
