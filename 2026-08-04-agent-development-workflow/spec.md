...
В рамках явно запрошенного пользователем real testing эта task вправе в exact bound destination создать или принять только утверждённые missing statuses `Human Review`, `Rework`, `Merging`, role labels, label `agent:codex`, требуемые Project statuses и bounded isolated acceptance Project family с его issues, relations, documents и comments. Family содержит ровно один complete-flow Project и один cancellation-path Project, потому что один terminal Project не может доказать обе Project semantics без запрещённого reopen. Existing standard statuses принимаются по semantic category и не переименовываются, не удаляются и не создаются повторно. Foreign labels, statuses, Projects и другие workspace settings не изменяются. Planned global delta всё равно показывается в execution output, но отдельное повторное разрешение на тот же exact closed mutation set не требуется.

Каждый acceptance Project получает user-visible name с префиксом `Acceptance` и provider-owned identity из exact team ID, source fingerprint, exact candidate fingerprint и path role `complete` либо `cancellation`. Retry одного non-terminal acceptance run reconciles тот же Project вместо создания duplicates. `Completed` и `Canceled` Projects никогда не reopen-ятся; повтор того же terminal candidate перечитывает сохранённое evidence, а другой source или candidate получает новую exact identity. Complete-flow Project и его issues сохраняются как `Completed` history, cancellation-path Project сохраняется как `Canceled` history. Cleanup удаляет только declared Git, local и external runtime resources и никогда не удаляет Linear history.
...
После handoff `task-graph-create` может применить user-approved Linear-native graph delta только к existing Project в `In Progress`, но не перечитывает later `project-goals/main` как новую версию исходного source. Delta provenance указывает на exact Linear decision, finding или manual source. Новые nodes сначала создаются в `Backlog` без `agent:codex`, затем получают все relations и проходят read-back; label добавляется пока node inactive, а `Todo` является её последней activation mutation. Delta сохраняет или расширяет downstream review, acceptance и final-cleanup blockers и не может обойти уже обязательную проверку. Existing unknown fields и unrelated nodes не перезаписываются. `Completed` или `Canceled` Project не reopen-ится этим workflow; новая independent work получает новый explicit source и отдельный Linear Project.

## Linear Project И Task Graph

`task-graph-create` преобразует один согласованный source в один Linear Project. Issues этого Project и blocker relations между ними образуют canonical task graph. Project хранит exact source provenance и import status. Source provenance не предполагает `project-goals`; ручные, incident, audit и monitoring sources используют тот же contract.
...
Публикуемый task graph не активируется с incomplete node: unresolved task-definition question возвращается к source authoring или оформляется полностью определённой `task:human`/evidence probe. `Backlog` внутри publication является staging state, а не способом спрятать незаданную работу в active Project.
