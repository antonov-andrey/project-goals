...
Multi-repository merge возобновляемый, но не атомарный. После interruption skill сравнивает каждый project main с target commit и продолжает только недостающие merges. Automatic revert и history rewrite запрещены. После всех merges skill разворачивает exact main commits в permanent primary development environment и выполняет полный applicable acceptance. При failure accepted pointer не меняется и durable merge journal сохраняет failed target. `goal-merge` не редактирует implementation source и не создаёт checkpoint скрыто; он возвращает exact failure handoff. Исправления получают новый separately authorized full fix-forward checkpoint через обычные implementation, `git-commit` и `goal-checkpoint` owners. Следующий `goal-merge` может выбрать его вместо failed target только после per-project ancestry proof, что новый snapshot включает каждый intervening commit; один invocation всё равно merge-ит и принимает ровно один selected checkpoint.
